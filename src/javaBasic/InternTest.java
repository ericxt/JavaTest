...
//		testLongLoop();
	...
	private static void testLongLoop() {
		test(1000 * 1000 * 1000);
		// uncomment the following line to see the hand-written cache
		// performance
	...
	private static void testStringPoolGarbageCollection() {
		...
		System.gc();
		// check the memory consumption and how long does it take to intern
		// strings
	...
	private static void test(final int cnt) {
		...
		for (int i = 0; i < cnt; ++i) {
			final String str = "Very long test string, which tells you about something "
					+ "very-very important, definitely deserving to be interned #"
					+ i;
			// uncomment the following line to test dependency from string
			// length
	...
	private static void testManual(final int cnt) {
		...
		for (int i = 0; i < cnt; ++i) {
			...
			if (i % 10000 == 0) {
				System.out.println(i + "; manual time = "
						+ (System.currentTimeMillis() - start) / 1000.0
						+ " sec");
				start = System.currentTimeMillis();
			}
